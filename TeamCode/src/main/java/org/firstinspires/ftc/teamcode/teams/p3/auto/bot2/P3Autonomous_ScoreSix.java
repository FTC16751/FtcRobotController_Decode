package org.firstinspires.ftc.teamcode.teams.p3.auto.bot2;
...
import org.firstinspires.ftc.teamcode.teams.p3.P3RobotConstants;
import org.firstinspires.ftc.teamcode.teams.p3.P3_Robot;
//import org.firstinspires.ftc.teamcode.common.SharedState;


@Autonomous(name="P3 AUTO Bot2: Score 6 (RUN ME)", group = "P3",preselectTeleOp = "P3: Teleop (Team Version)")
@Disabled
public class P3Autonomous_ScoreSix extends OpMode {
    ...
    private enum BlueFarState {START, MOVE_TO_SHOOTING_LOCATION, START_SHOOTING, SHOOTING, MOVE_TO_SPIKE_MARK3, INTAKE_ARTIFACTS_FROM_SPIKEMARK3, DRIVE_TO_SCORE_LINE, START_SHOOTING2, SHOOTING2, MOVE_TO_SPIKE_MARK2, INTAKE_ARTIFACTS_FROM_SPIKEMARK2, DRIVE_TO_SCORE_LINE_AGAIN, START_SHOOTING3, SHOOTING3, PARK }
    ...
    private void runBlueClosePath() {
        ...
        switch (blueCloseState) {
            ...
            case MOVE_AWAY_FROM_GOAL:
                robot.intake.startIntake();
                robot.launcher.setShooterMotorVelocity(autoTargetVelocity);
                if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_CLOSE_SHOOTING_POSITION, 0.5, .25)) {
                    shotsFired = 0;
                    blueCloseState = BlueCloseState.START_SHOOTING;
                }
                break;
            case START_SHOOTING:
                robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_CLOSE_SHOOTING_POSITION, 0.5, 0);
                robot.launcher.setShooterMotorVelocity(autoTargetVelocity);
                robot.launcher.setShootingPosition();
                blueCloseState = BlueCloseState.SHOOTING;
                break;
            case SHOOTING:
                robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_CLOSE_SHOOTING_POSITION, 0.55, 0);
            ...
            case INTAKE_ARTIFACTS_FROM_SPIKEMARK1:
                ...
                break;
            case DRIVE_TO_SCORE_LINE:
                if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_CLOSE_SHOOTING_POSITION, 0.5, 0.25))
                {
                    shotsFired = 0;
            ...
            case SHOOTING2:
                ...
                break;
            case PARK:
                if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_CLOSE_PARK, 0.5, .25)) {
                    autonomousState = AutonomousState.COMPLETE;
                }
                break;
        }
    ...
    private void runBlueFarPath() {
            ...
            switch (blueFarState) {
                ...
                case MOVE_TO_SHOOTING_LOCATION:
                    robot.intake.startIntake();
                    robot.launcher.setShooterMotorVelocity(1460);
                    if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.5, 0)) {
                        shotsFired = 0;
                        blueFarState = BlueFarState.START_SHOOTING;
                    }
                    break;
                case START_SHOOTING:
                    robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.5, 0);
                    autoTargetVelocity = 1460;//robot.getTargetVelocityForDistance(robot.vision.getDistanceToTagMeters()* 39.3701);
                    robot.launcher.setShooterMotorVelocity(autoTargetVelocity);
                    driveTimer.reset();
                ...
                case INTAKE_ARTIFACTS_FROM_SPIKEMARK3:
                    if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SPIKEMARK3_COLLECT, 0.5, .25)) {
                        ...
                        robot.feeder.setFeederMotorPower(-.45);
                    }
                    break;
                case DRIVE_TO_SCORE_LINE:
                    if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.5, 0)) {
                ...
                case SHOOTING2:
                    ...
                    if (shotsFired < 3) {
                        ...
                        // All 3 shots are done, move to the next auto state (e.g., PARK).
                        blueFarState = BlueFarState.MOVE_TO_SPIKE_MARK2;
                    }
                    break;
                ...
                case INTAKE_ARTIFACTS_FROM_SPIKEMARK2:
                    ...
                    break;
                case DRIVE_TO_SCORE_LINE_AGAIN:
                    if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.35, 0.0)) {
                        shotsFired = 0;
                        blueFarState = BlueFarState.START_SHOOTING3;
                    }
                    break;
                case START_SHOOTING3:
                    robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.5, 0);
                    autoTargetVelocity = 1460;//robot.getTargetVelocityForDistance(robot.vision.getDistanceToTagMeters()* 39.3701);
                    robot.launcher.setShooterMotorVelocity(autoTargetVelocity);
                    blueFarState = BlueFarState.SHOOTING3;
                    break;
                case SHOOTING3:
                    robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_SHOOTING_POSITION, 0.5, 0);
                    telemetry.addData("Path State", blueFarState);
                ...
                case PARK:
                    if (robot.drive.driveTo(robot.drive.pinpoint.getPosition(), P3RobotConstants.Bot2_Waypoints.BLUE_FAR_PARK_POSITION, 0.5, .25)) {
                        autonomousState = AutonomousState.COMPLETE;
                    }
                    break;
            }

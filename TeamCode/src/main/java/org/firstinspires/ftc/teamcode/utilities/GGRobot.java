...
public class GGRobot {
    ...
    /**
     * Executes a single AutoAction from a sequence by prioritizing drive actions.
    ...
    public void execute(AutoAction action) {
        // Determine if the action involves driving
        boolean needsToDrive = action.driveInches != 0 ||
                action.strafeInches != 0 ||
                action.turnDegrees != 0;

        // Determine if the action involves shooting
        boolean needsToShoot = action.feederSideToUse != null;

        // --- CORRECTED LOGIC: Prioritize drive actions ---
        if (needsToDrive) {
            // This is a drive action. Call the non-blocking drive method.
            drive.drive_p3(action.driveInches, action.strafeInches, action.turnDegrees, 0.5); // Using a default speed
        } else if (needsToShoot) {
            // This action is ONLY a shot. Call the non-blocking launch sequence.
            // We pass 'true' to initiate the shot.
            launchSequence(true, action.feederSideToUse, GGRobotConstants.LauncherDistance.FAR); // Provide a default distance
        }
        // If the action is empty, nothing will happen and isBusy() will be false,
        // allowing the OpMode to proceed to the next action immediately.
    }
